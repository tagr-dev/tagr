...
class TaggingTest(unittest.TestCase):
    ...
    def test_obj_name_to_dtype_conversion(self):
        ...
        self.assertEqual(self.tag.queue['X_train'].dtype, 'dataframe')

        # test aliasing
        test_int = 2
        self.tag.save(test_int, 'num', 'int')
        self.assertEqual(self.tag.queue['num'].dtype, 'primitive')

    def test_ignore_dupes(self):
        self.tag = Tagr()
        self.tag.save(2, 'num', 'int')
        self.tag.save(2, 'num', 'int')
        self.assertEqual(len(self.tag.queue), 1)

    def test_summary(self):
        data = [{'obj_name': 'num1', 'val': 2.0, 'dtype': 'primitive'},
